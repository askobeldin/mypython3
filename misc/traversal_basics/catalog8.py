...
def view_site(context, request):
    s = Template("""
                 ...
                 """)
    siteconfig = getattr(context, 'config', None)

    # menu = getsitemenu(request)

    output = s.safe_substitute(title = context.title,
                               keys = getFolderLeaves(request),
                               configdict = '%s' % siteconfig.items()
                                   if siteconfig else '',
                               menu = getsitemenu(request))
...
def view_catalogdocument(context, request):
    s = Template("""
                 ...
                 """)
    # eidt link should be like: .../db/20/@@edit
    catalogfolder = request.context.__parent__
    # print 'catalogfolder = %s' % catalogfolder
    catalog = catalogfolder.__parent__
    # print 'catalog = %s' % catalog
    catdb = find_resource(catalog, 'db')
    # print 'catdb = %s' % catdb
    editlink = request.resource_url(catdb, context.__name__, '@@edit')
...
def view_catalogdocument_edit(context, request):
    catalogfolder = request.context.__parent__
    catalog = catalogfolder.__parent__
    catdb = find_resource(catalog, 'db')
    db = catalog.db
    ...
    if request.method == 'POST':
        # handle forms data
        stag = request.POST.get('tag', None)
        sdata = request.POST.get('data', None)
        if (stag and sdata):
            # data filled
            print 'editing!'
            print 'stag = %s' % stag
            ...
            # save data
            db[context.__name__] = {'data': sdata,
                                    'tag': stag}

            # update all links in all subfolders
            catalogfolders = [i for i in catalog.values()
                              if isinstance(i, CatalogFolder)]
            for k in catalogfolders:
                k.updateDocuments()

            # redirect to edited document
            docurl = request.resource_url(catdb, context.__name__)
            return HTTPFound(docurl)
        ...
        s = Template("""
                     ...
                     -->
                         ...
                         <p>
                            <input type="text" maxlength="200" name="data" value="$docdata">
                         </p>
                         <p>
                     ...
                     """)
        curtag = db[context.__name__]['tag']
        tagslist = [('selected' if i == curtag else ' ',
                     i,
                     i.title()) for i in catalog.tags]
        selectoptions = ['<option %s value=\"%s\">%s</option>' % i
                         for i in tagslist]
...
if __name__ == '__main__':
    ...
    config.add_view(view=view_catalogdocument_edit,
                    name='edit',
                    context=CatalogDocument)
