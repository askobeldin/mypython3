...
if __name__ == '__main__':
    ...
    types = APPREGISTRY.get('dblist', None)

    if types:
        # make node in resource tree
        # tn = RTREE['types'] = Folder('types', RTREE, u'Types of toys')

        # search folder f1 from root of tree
        tn = find_resource(RTREE, 'f1')

        # generate documents of types
        for ttype in types.DBList:
            tn[ttype] = Document(name=ttype,
                                 parent=tn,
                                 title=u'Type: %s' % ttype,
                                 body=u'Body for type %s' % ttype)

        print 'resource_path(tn) is: ', resource_path(tn)

        fr = find_resource(tn, 'd2')
        print 'find_resource(tn, \'d2\') is: ', fr
        print 'and has title: %s' % fr.title

        # try to add additional data to document d1 in f1
        fd = find_resource(tn, 'd1')
        fd.body = '\n'.join([fd.body] + [x for x in types.DBList])
